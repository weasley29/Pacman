...
player_images = []
for i in range(1, 5):
    image = pygame.Surface((45, 45), pygame.SRCALPHA)
pygame.draw.circle(image, (255, 255, 0), (22, 22), 22)  # Draw a yellow Pacman circle
...
def draw_player():
    ...
    if direction == 0:
        frame = min(counter // 5, len(player_images) - 1)
        index = (counter // 5) % len(player_images)
        screen.blit(pygame.transform.rotate(player_images[index], 90), (player_x, player_y))   

    elif direction == 1:
        screen.blit(pygame.transform.flip(player_images[(counter // 5) % len(player_images)]
, True, False), (player_x, player_y))
    elif direction == 2:
        screen.blit(pygame.transform.rotate(player_images[(counter // 5) % len(player_images)]
, 90), (player_x, player_y))
    elif direction == 3:
        screen.blit(pygame.transform.rotate(player_images[(counter // 5) % len(player_images)]
, 270), (player_x, player_y))
...
while run:
    ...
    if counter < 19:
        counter += 1
        if counter >= len(player_images) * 5:
          counter = 0
        if counter > 3:
            flicker = False
    else:
